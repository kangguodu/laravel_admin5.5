...
{
  private $content_type = ['店鋪動態','平臺動態','規則更新','違規通知','營銷活動','重要通知'];
  private $sender;
  ...
  public function __construct()
  {
    ...
    public function index()
    {
      ...
      return Admin::content(function (Content $content) {
          ...
          {
            $button = '
            <a href="" class="disabled btn btn-sm btn-default">全部站内信</a>
            <a href="javascript:window.location.href=\''.url('platform/mail/create').'\'" class="btn btn-sm btn-default pull-right">新建站内信</a>';
    ...
    public function create()
    {
      ...
      return Admin::content(function (Content $content) {
          ...
          {
            $button = '
            <a href="'.url('platform/mail').'" class="btn btn-sm btn-default">全部站内信</a>
            <a href="javascript:window.location.href=\''.url('platform/mail/create').'\'" class="disabled btn btn-sm btn-default pull-right">新建站内信</a>';
            $row->Column(12,$button);
          });
          $content->row(function ($row)
          {
...
edo;
      ...
      return Admin::content(function (Content $content) use($arg) {
          ...
          $content->row(function ($row) use($arg)
          {
            ...
            foreach ($mailObj as $mail) {
              ...
              $handle .= '<a href="javascript:window.location.href=\''.url('platform/mail/'.$mail->id.'/edit').'\'" class="fa fa-pencil" title="修改"></a>&nbsp;&nbsp;&nbsp;&nbsp;';
              $handle .= '<a href="javascript:deleteMail('.$mail->id.')" class="fa fa-trash" title="刪除"></a>';
              $rows[] = [$mail->title,$this->content_type[$mail->content_type],@$this->sender[$mail->sender]['name'],date('Y-m-d H:i:s',$mail->send_time),@$this->sender[$mail->updated_by],empty($mail->updated_time)? null:date('Y-m-d H:i:s',$mail->updated_time),$handle];
              // array_unshift($rows,$row);
            }
            $table = new Table($headers,$rows);
    ...
    private function listUnread($id)
    {
      return Admin::content(function (Content $content) use($id) {
          $content->header('平臺站内信');
          $content->description('總覽');
          $btn = '<a href="'.url('platform/mail').'" class="btn btn-sm btn-default">全部站内信</a><a href="'.url('platform/mail/'.$id).'" class="btn btn-sm btn-default">站内信預覽</a><a href="'.url('platform/mail/'.$id.'-unread').'" class="disabled btn btn-sm btn-default">未讀店鋪</a><a href="'.url('platform/mail/'.$id.'-hadread').'" class="btn btn-sm btn-default">已讀店鋪</a><a href="javascript:window.location.href=\''.url('platform/mail/create').'\'" class="btn btn-sm btn-default pull-right">新建站内信</a>';
          $content->row($btn);
    ...
    private function listHadread($id)
    {
      return Admin::content(function (Content $content) use($id) {
          $content->header('平臺站内信');
          $content->description('總覽');
          $btn = '<a href="'.url('platform/mail').'" class="btn btn-sm btn-default">全部站内信</a><a href="'.url('platform/mail/'.$id).'" class="btn btn-sm btn-default">站内信預覽</a><a href="'.url('platform/mail/'.$id.'-unread').'" class="btn btn-sm btn-default">未讀店鋪</a><a href="'.url('platform/mail/'.$id.'-hadread').'" class="disabled btn btn-sm btn-default">已讀店鋪</a><a href="javascript:window.location.href=\''.url('platform/mail/create').'\'" class="btn btn-sm btn-default pull-right">新建站内信</a>';
          $content->row($btn);
    ...
    private function showMailContent($id)
    {
      return Admin::content(function (Content $content) use($id) {
          $content->header('平臺站内信');
          $content->description('總覽');
          $btn = '<a href="'.url('platform/mail').'" class="btn btn-sm btn-default">全部站内信</a><a href="'.url('platform/mail/'.$id).'" class="disabled btn btn-sm btn-default">站内信預覽</a><a href="'.url('platform/mail/'.$id.'-unread').'" class="btn btn-sm btn-default">未讀店鋪</a><a href="'.url('platform/mail/'.$id.'-hadread').'" class="btn btn-sm btn-default">已讀店鋪</a><a href="javascript:window.location.href=\''.url('platform/mail/create').'\'" class="btn btn-sm btn-default pull-right">新建站内信</a>';
          $content->row($btn);
          $content->row(function ($row) use($id)
          {
            $mailObj = MailList::find($id);
    ...
    private function getSelectRow($arg=null)
    {
      ...
      foreach ($this->content_type as $key => $type) {
        if(isset($arg['content_type'])&&$arg['content_type']==$key){
        $switch .= '<option selected="selected" value="'.$key.'">'.$type.'</option>';continue;}
      ...
      $sender .= '</select>';
      $submit = '<a href="javascript:mailSearchSubmit()" class="btn btn-sm btn-default"><i class="fa fa-search"></i>&nbsp;搜索</a>';
